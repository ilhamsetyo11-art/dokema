<x-admin-layouts>
    ...
    <div class="w-full md:w-7/12 xl:w-5/12 mx-auto mt-8 p-4 md:p-8 bg-white rounded shadow-md">
        ...
        <script>
            function fileUploadLampiranEdit(initialLampiran) {
                return {
                    file: null,
                    fileUrl: initialLampiran,
                    handleFileChange(e) {
        ...
        <form action="{{ route('laporan.update', [$magangId, $laporan->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-4">
                ...
                <select name="data_magang_id" id="data_magang_id" class="w-full border rounded px-3 py-2" required>
                    ...
                    @foreach ($dataMagangList as $dm)
                        <option value="{{ $dm->id }}" @if ($laporan->data_magang_id == $dm->id) selected @endif>{{ $dm->id }} - {{ $dm->profilPeserta->nama ?? 'Peserta' }} ({{ $dm->tanggal_mulai }} - {{ $dm->tanggal_selesai }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div x-data="fileUploadLampiranEdit('{{ $laporan->path_lampiran ? asset('storage/' . $laporan->path_lampiran) : '' }}')" class="mb-4">
                ...
                <div class="border-2 border-dashed border-gray-400 rounded p-6 flex flex-col items-center justify-center cursor-pointer bg-gray-50 hover:bg-gray-100 transition" @dragover.prevent @drop.prevent="handleDrop($event)" @click="$refs.lampiran.click()">
                    ...
                    <template x-if="fileUrl">
                        <div class="mb-2 w-full flex flex-col items-center">
                            <template x-if="file && file.type && file.type.startsWith('image/')">
                            ...
                            <template x-if="!file && fileUrl.endsWith('.pdf')">
                                <iframe :src="fileUrl" class="w-full h-40 mb-2 border rounded"></iframe>
                            </template>
                            <template x-if="!file && !fileUrl.endsWith('.pdf')">
                                <img :src="fileUrl" class="max-h-40 mb-2 rounded shadow" />
                            </template>
                            <div class="text-sm font-medium text-blue-700" x-text="file ? file.name : 'File sebelumnya' "></div>
                        </div>
                    </template>
                    <template x-if="!fileUrl">
